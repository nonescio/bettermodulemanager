 ...
 */
class BetterModulesModelModules extends JModelList
{
	/**
	 * Constructor.
	 *
		...
		foreach ($items as $item)
		{
			...
			if ($item->mirror_id > 0)
			{
				$item->menuid = JText::sprintf(
					'BMM_MIRRORING_MODULE',
					'[<a href="' . JRoute::_('index.php?option=com_bettermodules&task=module.edit&id=' . (int) $item->mirror_id) . '">'
					. $item->mirror_id . '</a>]'
				);
				continue;
			}
			...
			{
				$item->menuid = JText::sprintf(
					'BMM_MIRRORING_MODULE_OPPOSITE',
					'[<a href="' . JRoute::_('index.php?option=com_bettermodules&task=module.edit&id=' . (int) ($item->mirror_id * -1)) . '">'
					. ($item->mirror_id * -1) . '</a>]'
				);
				continue;
			}
	...
	protected function getListQuery()
	{
		...
		if (!empty($ids))
		{
			...
			$query->join('LEFT', '#__bettermodules AS aa ON aa.moduleid = a.id');
		...
		$query = $db->getQuery(true)
			...
			->join('LEFT', '#__bettermodules AS aa ON aa.moduleid = a.id')
			// Group
			->group(
