 ...
 * Module model.
 */
class BetterModulesModelModule extends JModelAdmin
{
	/**
	...
	protected function populateState()
	{
		...
		if (!$pk)
		{
			if ($extensionId = (int) $app->getUserState('com_bettermodules.add.module.extension_id'))
			{
		...
		// Load the parameters.
		$params = JComponentHelper::getParams('com_bettermodules');
	...
	protected function batchLanguage($value, $pks, $contexts)
	{
		...
		$db        = $this->getDbo();
		$table     = $this->getTable();
		$table_adv = JTable::getInstance('BetterModules', 'BetterModulesTable');
		...
		foreach ($pks as $pk)
		{
			if ($table->load($pk))
			{
				...
				else
				{
					...
					$query->clear()
						->delete('#__assets')
						->where('name = ' . $db->quote('com_bettermodules.module.' . (int) $pk));
					$db->setQuery($query);
					$db->execute();
	...
	public function getForm($data = array(), $loadData = true)
	{
		...
		// Get the form.
		$form = $this->loadForm('com_bettermodules.module', 'module', array('control' => 'jform', 'load_data' => $loadData));
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_bettermodules.edit.module.data', array());
		...
		if (empty($data))
		{
			...
			if (!$data->id)
			{
				$filters = (array) $app->getUserState('com_bettermodules.modules.filter');
			...
			if (empty($data->params))
			{
				...
				$params = $app->getUserState('com_bettermodules.add.module.params');
		...
		$this->preprocessData('com_bettermodules.module', $data);
	...
	public function getItem($pk = null)
	{
		...
		if (!isset($this->_cache[$pk]))
		{
			...
			if (empty($pk))
			{
				...
				{
					$app = JFactory::getApplication();
					$app->redirect(JRoute::_('index.php?option=com_bettermodules&view=modules', false));
			...
			// Advanced parameters
			// Get a row instance.
			$table_adv = $this->getTable('BetterModules', 'BetterModulesTable');
	...
	protected function preprocessForm(JForm $form, $data, $group = 'content')
	{
		...
		JForm::addFormPath(JPATH_ADMINISTRATOR . '/components/com_bettermodules/models/forms');
	...
	public function save($data)
	{
		...
		$query = $db->getQuery(true)
			->delete('#__assets')
			->where('name = ' . $db->quote('com_bettermodules.module.' . (int) $table->id));
		$db->setQuery($query);
		$db->execute();
	...
	public function saveAdvancedParams($data, $id = 0)
	{
		...
		require_once JPATH_ADMINISTRATOR . '/components/com_bettermodules/tables/bettermodules.php';
		$table_adv           = JTable::getInstance('BetterModules', 'BetterModulesTable');
	...
	public function initAssignments($id, &$module)
	{
		...
		if (empty($id))
		{
			...
			BetterModulesModelModule::saveAdvancedParams($module->advancedparams, $id);
	...
	{
		parent::cleanCache('com_bettermodules', $this->getClient());
	}
	...
	private function getConfig()
	{
		...
		$this->config = $parameters->getComponentParams('bettermodules');
