...
class Com_BetterModulesInstallerScript extends Com_BetterModulesInstallerScriptHelper
{
	...
	public function onBeforeInstall()
	{
		...
		$this->fixFileVersions(
			array(
				JPATH_ADMINISTRATOR . '/components/com_bettermodules/bettermodules.xml',
				JPATH_PLUGINS . '/system/bettermodules/bettermodules.xml',
			)
		);
	}
	...
	{
		// main table
		$query = "CREATE TABLE IF NOT EXISTS `#__bettermodules` (
	...
	{
		// add asset_id column
		$query = "SHOW COLUMNS FROM `" . $this->db->getPrefix() . "bettermodules` LIKE 'asset_id'";
		$this->db->setQuery($query);
		...
		$query = "ALTER TABLE `#__bettermodules` ADD `asset_id` INT(10) UNSIGNED NOT NULL DEFAULT '0' AFTER `moduleid`";
		$this->db->setQuery($query);
		$this->db->execute();
	}
	...
	private function fixMirrorIdField()
	{
		...
		$query = "SHOW COLUMNS FROM `" . $this->db->getPrefix() . "bettermodules` LIKE 'mirror_id'";
		$this->db->setQuery($query);
		...
		$query = "ALTER TABLE `#__bettermodules` ADD `mirror_id` INT(10) NOT NULL DEFAULT '0' AFTER `asset_id`";
		$this->db->setQuery($query);
		$this->db->execute();
	...
	private function fixMirrorIdFieldFixParams()
	{
		...
		$query = $this->db->getQuery(true)
			...
			->from($this->db->quoteName('#__bettermodules'));
		$this->db->setQuery($query);
		...
		foreach ($rows as $row)
		{
			...
			$query->clear()
				->update($this->db->quoteName('#__bettermodules'))
	...
	private function removeAdminMenu()
	{
		...
		$query = $this->db->getQuery(true)
			...
			->where($this->db->quoteName('path') . ' = ' . $this->db->quote('bettermodules'))
	...
	private function removeFrontendComponentFromDB()
	{
		...
		$query = $this->db->getQuery(true)
			...
			->where($this->db->quoteName('element') . ' = ' . $this->db->quote('com_bettermodules'))
			->where($this->db->quoteName('type') . ' = ' . $this->db->quote('component'))
	...
	private function deleteOldFiles()
	{
		JFile::delete(
			array(
				JPATH_ADMINISTRATOR . '/components/com_bettermodules/script.bettermodules.php',
				JPATH_SITE . '/components/com_bettermodules/bettermodules.xml',
				JPATH_SITE . '/components/com_bettermodules/script.bettermodules.php',
				JPATH_SITE . '/plugins/system/bettermodules/modulehelper.php',
			)
		);
	}
	...
	public function fixAssetsRules($rules = '')
	{
		...
		$query = $this->db->getQuery(true)
			->delete('#__assets')
			->where('name LIKE ' . $this->db->quote('com_bettermodules.module.%'));
		$this->db->setQuery($query);
		$this->db->execute();
	}
