...
class PlgSystemBetterModuleHelper
{
	...
	public function onPrepareModuleList(&$modules)
	{
		...
		require_once JPATH_ADMINISTRATOR . '/components/com_bettermodules/models/module.php';
		$model = new BetterModulesModelModule;

		$xmlfile_assignments = JPATH_ADMINISTRATOR . '/components/com_bettermodules/assignments.xml';
		...
		foreach ($modules as $module)
		{
			...
			BetterModules::setCurrentModule($module);
	...
	private function getModuleList()
	{
		...
		$query = $db->getQuery(true)
			...
			->join('LEFT', '#__bettermodules as am ON am.moduleid = m.id')
			->where('m.published = 1')
			->where('e.enabled = 1')
	...
	private function getConfig()
	{
		...
		require_once JPATH_LIBRARIES . '/regularlabs/helpers/parameters.php';
		$parameters = RLParameters::getInstance();
		$instance   = $parameters->getComponentParams('bettermodules');
