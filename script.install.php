...
class PlgSystemBetterInstallerInstallerScript
{
	...
	{
		JFactory::getLanguage()->load('plg_system_betterinstaller', __DIR__);
	...
	{
		if (!JFolder::exists(JPATH_SITE . '/plugins/system/betterinstaller'))
		{
			return;
		}
		...
		$this->deleteFolders(
			array(
				JPATH_SITE . '/plugins/system/betterinstaller/language',
				JPATH_SITE . '/plugins/system/betterinstaller',
			)
		);
		...
		$query = $db->getQuery(true)
			->delete('#__extensions')
			->where($db->quoteName('element') . ' = ' . $db->quote('betterinstaller'))
	...
	public function requiresOldFramework()
	{
		...
		$extensions = array(
			array('6', '/administrator/components/com_bettermodules/bettermodules.xml'),
